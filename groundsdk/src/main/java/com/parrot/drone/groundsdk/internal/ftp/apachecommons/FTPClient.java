package com.parrot.drone.groundsdk.internal.ftp.apachecommons;
...
import com.parrot.drone.groundsdk.internal.ftp.FtpSession.CreateProxyListener;
import com.parrot.drone.groundsdk.internal.ftp.FtpSession.RemoveProxyListener;
import com.parrot.drone.groundsdk.internal.ftp.apachecommons.parser.DefaultFTPFileEntryParserFactory;
import com.parrot.drone.groundsdk.internal.ftp.apachecommons.parser.FTPFileEntryParserFactory;
import com.parrot.drone.groundsdk.internal.ftp.apachecommons.parser.MLSxEntryParser;
        ...
        implements Configurable
{
     ...
     * @since 3.0
     */
    public static final String FTP_SYSTEM_TYPE = "systemType";
     ...
     * @since 3.1
     */
    public static final String FTP_SYSTEM_TYPE_DEFAULT = "systemType.default";
     ...
     * <pre>
     * Plan 9=Unix
     * OS410=parser.OS400FTPEntryParser
     * </pre>
     *
     * @since 3.0
     */
    ...
    public FTPClient(@Nullable final CreateProxyListener createProxyListener, @Nullable RemoveProxyListener removeProxyListener) {
        __initDefaults();
        __dataTimeout = -1;
     ...
     *
     * @see .parser.FTPFileEntryParserFactory
     * @see .parser.DefaultFTPFileEntryParserFactory
     */
     ...
     *                   from the server.
     * @throws .parser.ParserInitializationException
    ...
    // proxy chain
    private void createProxy() throws IOException {
        if (createProxyListener != null) {
            new Handler(Looper.getMainLooper()).post(() -> createProxyListener.onCreateProxy(__passivePort, this::onProxyCreated));

            final long timeoutMillis = System.currentTimeMillis() + super.getConnectTimeout();

            while (!proxyCreated && timeoutMillis > System.currentTimeMillis()) {
                SystemClock.sleep(1);
            }
            ...
            if (timeoutMillis <= System.currentTimeMillis()) {
                throw new IOException("timed out waiting for proxy creation");
            }
        }
    }

    private void onProxyCreated(String address, int port, SocketFactory socketFactory) {
        proxiedPort = __passivePort;
        __passivePort = port;
        proxyCreated = true;
    }

    private void removeProxy() {
        if (removeProxyListener != null && proxiedPort != -1) {
            removeProxyListener.onRemoveProxy(proxiedPort);
            proxiedPort = -1;
        }
    }
}
